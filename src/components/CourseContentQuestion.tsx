...
const CourseContentQuestion: React.FC<Props> = ({
  ...
  if (!quiz) {
    return (
      <div className="border border-[rgba(0,0,0,0.25)] min-h-[300px] rounded-lg p-4">
    ...
    <div className="border border-[rgba(0,0,0,0.25)] min-h-[300px] rounded-lg p-4">
      ...
      <div className="flex justify-end gap-2">
        {submitted && !isCorrect && (
          <button
            onClick={handleRetry}
            className="px-4 py-2 text-xs rounded cursor-pointer hover:bg-gray-50"
          >
            Try Again
          </button>
        )}
        <button
          ...
          className="px-4 py-2 text-xs bg-[#ff9801] hover:bg-[#e68a00] cursor-pointer rounded disabled:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Submit Answer
        </button>
      </div>
